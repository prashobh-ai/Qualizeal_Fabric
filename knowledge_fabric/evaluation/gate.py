...
THRESHOLDS = {
    ...
    "grounding_pass_rate": 0.6,
    "answer_supportedness": 0.75,
    "recall": 0.5,
}
...
def _eval_principal(platform, tenant: str) -> Principal:
    token = platform.idp.mint(
        Principal(
            subject="eval-bot", tenant=tenant, roles=["asker"], scopes=["public", "restricted"]
        )
    )
...
def evaluate(platform, tenant: str, candidate_version: int = 1) -> dict:
    ...
    platform.db.execute(
        ...
        "VALUES(?,?,?,?,?,?,?)",
        (
            f"eval_{tenant}_{candidate_version}_{now_ms()}",
            tenant,
            candidate_version,
            1 if passed else 0,
            str(metrics),
...
def promote_if_passes(platform, tenant: str, candidate_version: int) -> dict:
    ...
    if verdict["passed"]:
        platform.db.execute(
            "UPDATE index_versions SET state='retired' WHERE tenant=? AND state='live'", (tenant,)
        )
        platform.db.execute(
            (
                "INSERT OR REPLACE INTO index_versions(tenant,version,state,promoted_at) "
                "VALUES(?,?,?,?)"
            ),
        ...
        platform.curation.add(
            tenant, f"promotion blocked: {verdict['regressions']}", "gap", now_ms()
        )
    return verdict
