...
class SqlQueue:
    ...
    def enqueue(self, tenant: str, job: Job) -> str:
        self.db.execute(
            "INSERT INTO jobs(id,tenant,kind,payload,state,attempts,lease,dead_letter_reason,"
            "created_at)\n               VALUES(?,?,?,?,?,?,?,?,?)",
            (
                job.id,
                tenant,
    ...
    def _reclaim_expired(self) -> None:
        ...
        self.db.execute(
            (
                "UPDATE jobs SET state='pending', lease=NULL WHERE state='leased' AND lease IS NOT "
                "NULL AND lease < ?"
            ),
            (now,),
        )
    ...
    def lease(self, worker: str, ttl_s: float = 30.0) -> Job | None:
        with self.db._lock:
            ...
            r = self.db.one("SELECT * FROM jobs WHERE state='pending' ORDER BY created_at LIMIT 1")
            if not r:
                return None
            lease_until = time.time() + ttl_s
            self.db.execute(
                (
                    "UPDATE jobs SET state='leased', lease=?, attempts=attempts+1 WHERE id=? AND "
                    "state='pending'"
                ),
                (lease_until, r["id"]),
            )
    ...
    def deadletter(self, job_id: str, reason: str) -> None:
        self.db.execute(
            "UPDATE jobs SET state='dead', lease=NULL, dead_letter_reason=? WHERE id=?",
            (reason, job_id),
        )
