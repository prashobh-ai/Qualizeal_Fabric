...
def set_schedule(
    ...
    next_run = (
        existing["last_run"] + interval
        if existing is not None and existing["last_run"] is not None
        else now
    )
    platform.db.execute(
...
def get_schedule(platform, tenant: str, source: str) -> dict | None:
    _guard(tenant)
    _check_source(source)
    r = platform.db.one(
        "SELECT * FROM refresh_schedules WHERE tenant=? AND source=?", (tenant, source)
    )
...
def remove_schedule(platform, tenant: str, source: str) -> bool:
    ...
    cur = platform.db.execute(
        "DELETE FROM refresh_schedules WHERE tenant=? AND source=?", (tenant, source)
    )
    return bool(cur.rowcount)


def enable_schedule(platform, tenant: str, source: str, enabled: bool) -> dict | None:
    """Pause/resume a schedule without touching its interval or history."""
    _guard(tenant)
    _check_source(source)
    platform.db.execute(
        "UPDATE refresh_schedules SET enabled=? WHERE tenant=? AND source=?",
        (1 if enabled else 0, tenant, source),
    )
    return get_schedule(platform, tenant, source)


def due(platform, tenant: str, now: float | None = None) -> list[str]:
...
def _update_schedule(platform, tenant: str, source: str, **fields) -> None:
    ...
    platform.db.execute(
        f"UPDATE refresh_schedules SET {cols} WHERE tenant=? AND source=?",
...
def _execute(
    ...
    if not admin.is_enabled(platform, tenant, source):
        if sched:
            ...
            _update_schedule(
                platform, tenant, source, next_run=base["next_run"], last_status=_SKIPPED_DISABLED
            )
    ...
    try:
        ...
        runs.step(
            ...
            run_id,
            "tombstone",
            "ok" if summary["tombstoned"] else "skipped",
        ...
        if sched:
            ...
            _update_schedule(
                ...
                last_status=("error:" + msg)[:_MAX_STATUS],
                error_count=error_count,
            )
...
def run_due(
    platform, tenant: str, now: float | None = None, records_by_source: dict | None = None
) -> list[dict]:
...
# --------------------------------------------------------------------------
def _health_entry(
    platform, tenant: str, source: str, sched: dict | None, cursor, now: float
) -> dict:
    last_ok = [
        t
        for t in (
            sched["last_run"] if sched else None,
            (cursor["last_sync"] / 1000.0)
            if (cursor is not None and cursor["last_sync"])
            else None,
        )
        if t is not None
    ]
...
def health(platform, tenant: str, now: float | None = None) -> list[dict]:
    ...
    return [
        _health_entry(platform, tenant, s, scheds.get(s), cursors.get(s), now)
        for s in sorted(set(scheds) | set(cursors))
    ]


def health_for(platform, tenant: str, source: str, now: float | None = None) -> dict:
    ...
    return _health_entry(
        platform, tenant, source, get_schedule(platform, tenant, source), cursor, _now(now)
    )
...
class RefreshLoop:
    ...
    def __init__(
        self, platform, tenant: str, tick_s: float = 5, records_by_source: dict | None = None
    ):
        _guard(tenant)
        if float(tick_s) <= 0:
    ...
    def start(self) -> bool:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._loop, daemon=True, name=f"kf-refresh-{self.tenant}"
            )
            self._thread.start()
            return True
