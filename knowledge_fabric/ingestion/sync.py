...
class SyncManager:
    ...
    def _cursor(self, tenant, source):
        r = self.p.db.one(
            "SELECT cursor FROM connector_cursors WHERE tenant=? AND source=?", (tenant, source)
        )
    ...
    def sync(
        self, tenant: str, source: str, config: dict, ontology: str = "quality-assurance", **kwargs
    ) -> dict:
        ...
        for spec in sources:
            ...
            out.append(
                self.sync(
                    ...
                    spec["source"],
                    spec.get("config", {}),
                    spec.get("ontology", "quality-assurance"),
                    **kw,
                )
            )
        return out
    ...
    def source_health(self, tenant: str) -> list[dict]:
        rows = self.p.db.query(
            "SELECT source, cursor, last_sync, items FROM connector_cursors WHERE tenant=?",
            (tenant,),
        )
        now = now_ms()
        return [
