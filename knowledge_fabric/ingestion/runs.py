...
__all__ = [
    "STEP_STATUSES", "RUN_STATUSES", "RUNNING", "PIPELINE_STAGES",
    "start_run", "step", "finish", "list_runs", "get_run", "active_runs",
    "rollup", "active", "current_run_id", "step_if_active", "step_from_span",
]
...
def _tenant_of(run_id: str, tenant: Optional[str]) -> str:
    if tenant:
...
def _row(r) -> dict:
    ...
    return {
        "id": r["id"], "source": r["source"], "started_at": started, "finished_at": finished,
        "status": r["status"], "steps": steps, "items": int(r["items"] or 0),
        "stages": rollup(steps),
...
def step(platform, run_id: str, name: str, status: str, count: int = 0, ms: float = 0.0,
         detail: Optional[str] = None, tenant: Optional[str] = None) -> None:
    ...
    db = _db(platform)
    entry: dict[str, Any] = {"name": name, "status": status, "count": int(count),
                             "ms": round(float(ms), 2), "at": now_ms()}
    if detail:
    ...
    with _steps_lock:
        ...
        steps.append(entry)
        db.execute("UPDATE ingest_runs SET steps=? WHERE tenant=? AND id=?",
                   (json.dumps(steps), t, run_id))


def finish(platform, run_id: str, status: str, items: int, tenant: Optional[str] = None) -> None:
    ...
    with _steps_lock:
        ...
        if row["status"] != RUNNING:
            raise ValueError(f"run '{run_id}' already finished with status {row['status']!r}")
        db.execute("UPDATE ingest_runs SET finished_at=?, status=?, items=? WHERE tenant=? AND id=?",
...
def get_run(platform, tenant: str, run_id: str) -> Optional[dict]:
...
def active_runs(platform, tenant: str) -> list[dict]:
    ...
    _guard(tenant)
    rows = _db(platform).query(
        "SELECT * FROM ingest_runs WHERE tenant=? AND status=? ORDER BY started_at DESC, rowid DESC",
        (tenant, RUNNING))
    return [_row(r) for r in rows]
...
def rollup(steps: list[dict]) -> list[dict]:
    ...
    for s in steps or []:
        ...
        cur = out.get(name)
        if cur is None:
            cur = out[name] = {"name": name, "status": "skipped", "count": 0, "ms": 0.0,
                               "entries": 0, "last_at": None}
...
def step_if_active(platform, name: str, status: str = "ok", count: int = 0, ms: float = 0.0,
