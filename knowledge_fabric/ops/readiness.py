...
ENV_SPEC: list[dict] = [
    {
        "var": "KF_DB_URL",
        "local": "unset (SQLite via KF_DB)",
        "aws": "postgres://… from Secrets Manager (terraform output db_secret_arn)",
        "required_aws": True,
        "secret": True,
    },
    {
        "var": "KF_DB",
        "local": ":memory: or /data/kf.db",
        "aws": "unused once KF_DB_URL is postgres",
        "required_aws": False,
        "secret": False,
    },
    {"var": "KF_OBJECTSTORE", "local": "local", "aws": "s3", "required_aws": True, "secret": False},
    {
        "var": "KF_S3_BUCKET",
        "local": "—",
        "aws": "terraform output s3_bucket",
        "required_aws": True,
        "secret": False,
    },
    {
    ...
    },
    {
        "var": "KF_BLOBS",
        "local": "./data/blobs",
        "aws": "unused (container scratch only)",
        "required_aws": False,
        "secret": False,
    },
    {"var": "KF_QUEUE", "local": "local", "aws": "sqs", "required_aws": True, "secret": False},
    {
        "var": "KF_SQS_URL",
        "local": "—",
        "aws": "terraform output sqs_queue_url",
        "required_aws": True,
        "secret": False,
    },
    {
        "var": "KF_SQS_DLQ_URL",
        "local": "—",
        "aws": "terraform output sqs_dlq_url",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "AWS_REGION",
        "local": "—",
        "aws": "var.region (task env)",
        "required_aws": True,
        "secret": False,
    },
    {
        "var": "KF_IDP_SECRET",
        "local": DEV_IDP_SECRET,
        "aws": "Secrets Manager (terraform output idp_secret_arn)",
        "required_aws": True,
        "secret": True,
    },
    {
        "var": "KF_OIDC_ISSUER",
        "local": "kf-local (built-in IdP)",
        "aws": "Cognito user-pool issuer or var.oidc_issuer",
        "required_aws": True,
        "secret": False,
    },
    {
        "var": "KF_OIDC_AUDIENCE",
        "local": "knowledge-fabric",
        "aws": "Cognito app-client id or var.oidc_audience",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "KF_MODEL_MODE",
        "local": "mock",
        "aws": "hosted | self-hosted | off (var.model_mode)",
        "required_aws": True,
        "secret": False,
    },
    {
        "var": "KF_MODEL_BASE_URL",
        "local": "—",
        "aws": "model gateway URL (var.model_base_url)",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "KF_MODEL_API_KEY",
        "local": "—",
        "aws": "Secrets Manager (terraform output model_key_secret_arn)",
        "required_aws": False,
        "secret": True,
    },
    {
        "var": "KF_MODEL_FAST",
        "local": "kf-mock-small",
        "aws": "named model per tier (optional)",
        "required_aws": False,
        "secret": False,
    },
    {
    ...
    {
        "var": "KF_GROUNDING_THRESHOLD",
        "local": "0.50",
        "aws": "var.grounding_threshold",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "KF_BUDGET_CAP_USD",
        "local": "—",
        "aws": "var.budget_cap_usd (reserved: the cap is set via POST /admin/budget today)",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "KF_PORT",
        "local": "8080",
        "aws": "8080 (var.container_port)",
        "required_aws": False,
        "secret": False,
    },
    {
        "var": "KF_OTLP_ENDPOINT",
        "local": "—",
        "aws": "OTLP collector endpoint (optional)",
        "required_aws": False,
        "secret": False,
    },
]
...
_PLACEHOLDER = re.compile(
    r"(?i)\$\{|<|>|example|change-?me|placeholder|redacted|\*\*\*|your[_-]|xxx|dummy"
)


def _check(id_: str, name: str, status: str, detail: str, items: list | None = None) -> dict:
...
def check_python() -> dict:
    ...
    if (v.major, v.minor) >= (3, 11):
        return _check(
            ...
            "Python interpreter",
            "ok",
            f"Python {ver} (deploy/Dockerfile pins python:3.11-slim)",
        )
    return _check(
        "python",
        "Python interpreter",
...
def check_env(target: str, env: dict) -> dict:
    ...
    if target == "local":
        detail = (
            f"{len(present)}/{len(ENV_SPEC)} known KF_* variables set; every one has a local "
            f"default"
        ) + (": " + ", ".join(f"{v}={_redact(v, env[v])}" for v in present) if present else "")
        if env.get("KF_MODEL_MODE", "mock").lower() == "hosted" and not (
            env.get("KF_MODEL_BASE_URL") and env.get("KF_MODEL_API_KEY")
        ):
            items.append(
                "KF_MODEL_MODE=hosted without KF_MODEL_BASE_URL/KF_MODEL_API_KEY falls back to "
                "the mock model"
            )
    ...
    if env.get("KF_MODEL_MODE", "").lower() == "hosted" and not (
        env.get("KF_MODEL_BASE_URL") and env.get("KF_MODEL_API_KEY")
    ):
        wrong.append(
            "KF_MODEL_MODE=hosted requires KF_MODEL_BASE_URL and KF_MODEL_API_KEY (Secrets Manager)"
        )
    items.extend(wrong)
    n_set = len(REQUIRED_AWS) - sum(1 for v in REQUIRED_AWS if not env.get(v))
    detail = f"{n_set}/{len(REQUIRED_AWS)} required variables set" + (
...
def check_adapters(target: str, env: dict) -> dict:
    ...
    picked = (
        f"objectstore={sel['objectstore']['adapter']} queue={sel['queue']['adapter']} "
        f"database={sel['database']['adapter']}"
    )
    items = [f"{k}: {m}" for k in ("objectstore", "queue", "database") for m in sel[k]["missing"]]
    if target == "local":
        cloudy = [
            k
            for k in ("objectstore", "queue", "database")
            if sel[k]["mode"] not in ("local", "sqlite")
        ]
        status = "blocker" if items else ("warn" if cloudy else "ok")
        detail = picked + (
            " (cloud adapters selected on a local target)" if cloudy else " (local defaults)"
        )
...
def check_libraries(target: str) -> dict:
    ...
    have = [
        ...
        for n, ok in (
            ("boto3", libs["boto3"]),
            (libs["pg_driver"] or "pg-driver", bool(libs["pg_driver"])),
        )
        if ok
    ]
    detail = (
        ...
        + " (the image is stdlib-only by default)"
    )
    if target == "local":
        return _check(
            "libraries", "Optional client libraries", "ok", detail + "; none needed locally"
        )
    items = []
    if not libs["boto3"]:
    ...
    return _check(
        "libraries", "Optional client libraries", "blocker" if items else "ok", detail, items
    )
...
def check_stdlib(repo_root: Path) -> dict:
    ...
    detail = (
        f"{len(list(pkg.rglob('*.py')))} modules scanned; unguarded third-party imports: "
        f"{len(offenders)}"
    )
    return _check(
        "stdlib", "Standard-library-only image", "warn" if offenders else "ok", detail, offenders
    )
...
def check_image(repo_root: Path) -> dict:
    ...
    if not df.exists():
        return _check(
            "image",
            "Container image build file",
            "blocker",
            "deploy/Dockerfile missing",
            ["deploy/Dockerfile"],
        )
...
def tf_variable_problems(iac_dir: Path) -> tuple[list[str], list[str]]:
    ...
    if vf.exists():
        declared = set(
            re.findall(r'^variable\s+"([A-Za-z0-9_]+)"', vf.read_text(encoding="utf-8"), re.M)
        )
    used: set[str] = set()
...
def check_iac(target: str, repo_root: Path) -> dict:
    ...
    if missing:
        ...
        return _check(
            ...
            [f"deploy/aws/{f}" for f in missing],
        )
    items: list[str] = []
    ...
    for tf in sorted(iac.glob("*.tf")):
        for m in re.finditer(
            r'(?im)^\s*(password|secret_string|api_key|[a-z_]*secret)\s*=\s*"([^"$]{8,})"',
    ...
    n_vars = len(
        re.findall(r"^variable ", (iac / "variables.tf").read_text(encoding="utf-8"), re.M)
    )
    n_outs = len(re.findall(r"^output ", (iac / "outputs.tf").read_text(encoding="utf-8"), re.M))
    detail = (
        f"{len(list(iac.glob('*.tf')))} .tf files, {len(types)} resource types, "
        f"{n_vars} variables, "
        f"{n_outs} outputs; "
    ...
    if items:
        return _check(
            "iac",
            "Infrastructure as code (deploy/aws)",
            "blocker" if target == "aws" else "warn",
            detail,
            items,
        )
    return _check(
...
def scan_secrets(repo_root: Path, max_bytes: int = 2_000_000) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root):
        ...
        for fn in sorted(filenames):
            p = Path(dirpath) / fn
            if fn.endswith(
                (".db", ".db-wal", ".db-shm", ".pyc", ".png", ".jpg", ".gif", ".pdf", ".zip")
            ):
                continue
            try:
...
def check_secrets(target: str, repo_root: Path) -> dict:
    ...
    if dotenv.exists() and dotenv.stat().st_size > 0:
        items.append(
            ...
            + (" (ignored by git)" if ".env" in ignored else " and NOT ignored")
        )
    status = "blocker" if hits or (target == "aws" and items) else ("warn" if items else "ok")
    detail = (
        f"{len(SECRET_PATTERNS)} credential patterns grepped across the repository; hits: "
        f"{len(hits)}"
    )
...
def check_identity(target: str, env: dict) -> dict:
    ...
    if secret == DEV_IDP_SECRET:
        items.append(
            "KF_IDP_SECRET is the development default"
            + ("" if target == "local" else " — inject from Secrets Manager")
        )
    elif len(secret) < 32:
...
def check_health(live: bool, repo_root: Path) -> dict:
    ...
    try:
        ...
        return _check(
            ...
            "Health endpoint",
            "blocker",
            f"live probe failed: {e.__class__.__name__}: {e}",
...
def check_tests(repo_root: Path, run: bool, timeout_s: float = 900.0) -> dict:
    if not run:
        return _check(
            ...
            "skip",
            (
                "not run (--skip-tests); run: PYTHONPATH=. python3 -m unittest "
    ...
    return _check(
        ...
        "blocker",
        f"FAILED: {ran}",
        [f"test suite red (exit {proc.returncode}): {tail}"],
    )
...
def report(
    target: str = "local",
    repo_root: os.PathLike | None = None,
    env: dict | None = None,
    ...
    summary = {
        s: sum(1 for c in checks if c["status"] == s) for s in ("ok", "warn", "blocker", "skip")
    }
    missing = [
        it for c in checks if c["status"] == "blocker" for it in (c["items"] or [c["detail"]])
    ]
    mapping = [
        {
            "var": e["var"],
            "local": e["local"],
            "aws": e["aws"],
            "required_aws": e["required_aws"],
            "present": bool(env.get(e["var"])),
            "value": _redact(e["var"], env[e["var"]]) if env.get(e["var"]) else None,
        }
        for e in ENV_SPEC
    ]
    return {
...
def render(rep: dict) -> str:
    lines = [
        f"Knowledge Fabric readiness — target: {rep['target']}",
    ...
    lines += [
        "",
        f"adapters: objectstore={a['objectstore']['adapter']} ({a['objectstore']['mode']}) "
    ...
    lines += [
        "",
        (
            f"Result: {verdict} — {s['blocker']} blocker(s), {s['warn']} warning(s), {s['ok']} ok, "
            f"{s['skip']} skipped"
        ),
    ]
    return "\n".join(lines)
