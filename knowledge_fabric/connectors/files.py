    ...
    def pull(self, cursor: Optional[str]) -> tuple[list[RawItem], Optional[str]]:
        ...
        for path in sorted(folder.glob("**/*")):
            ...
            items.append(RawItem(
                tenant=self.tenant, source=self.source_name, source_version=str(int(mtime)),
                uri=f"file://{path.name}", mime="text/plain", title=path.stem,
                bytes_=path.read_bytes(), meta={"acl": self.config.get("acl", ["public"])}))
