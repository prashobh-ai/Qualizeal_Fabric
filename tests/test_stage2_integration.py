...
from knowledge_fabric.adapters.identity import (
    LocalIdP,
    OIDCIdentity,
    OIDCNotReadyError,
    build_identity,
...
class TestAnswerPathFixes(unittest.TestCase):
    ...
    def test_complexity_not_lowered_by_model_off(self):
        ...
        on = self.svc.ask(self.asker, q)
        self.p.cache.invalidate(T)
        self.p.model = DisabledModelClient()
        off = self.svc.ask(self.asker, q)
        self.assertEqual(off.tier, "none")
        self.assertEqual(
            off.complexity, on.complexity, "complexity describes the query, not the tier"
        )
    ...
    def test_reasoned_answer_is_cached(self):
        ...
        a1 = self.svc.ask(self.asker, q)
        self.assertEqual((a1.reasoning or {}).get("mode"), "multistep")
        a2 = self.svc.ask(self.asker, q)
        self.assertTrue(a2.cache_hit)
        self.assertEqual((a2.reasoning or {}).get("mode"), "multistep")
...
class TestIdentitySelection(unittest.TestCase):
    def test_default_local_and_oidc_selection(self):
        ...
        o = build_identity(
            {
                "KF_IDENTITY": "oidc",
                "KF_OIDC_ISSUER": "https://idp.example.com/x",
                "KF_OIDC_AUDIENCE": "kf",
            },
            "s",
        )
    ...
    def test_oidc_fails_closed_without_verifier_or_valid_token(self):
        ...
        with self.assertRaises((OIDCNotReadyError, PermissionError, Exception)):
...
class TestHttpFixes(unittest.TestCase):
    ...
    def test_versions_diff_route(self):
        ...
        doc_id = docs["documents"][0]["document_id"]
        code, out = self._call(
            "GET", f"/curator/versions?document_id={doc_id}&from=1&to=1", "curator"
        )
        self.assertEqual(code, 200)
        self.assertIn("unchanged", out["diff"])

    def test_otlp_dry_run_route(self):
        code, out = self._call("GET", "/admin/otlp", "admin")
        self.assertEqual(code, 200)
        self.assertIn("resourceSpans", json.dumps(out))
...
class TestRefreshLoops(unittest.TestCase):
    def test_start_and_stop_loops(self):
        ...
        try:
            self.assertTrue(loops and all(lp.running for lp in loops))
        finally:
            for lp in loops:
                lp.stop()
            os.environ.pop("KF_REFRESH_TICK_S", None)
        self.assertFalse(any(lp.running for lp in loops))
