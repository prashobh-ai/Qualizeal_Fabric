...
class TestBankGeneration(unittest.TestCase):
    ...
    def test_generate_produces_gated_entries(self):
        ...
        loaded_uris = {
            (d.get("uri") or "").replace("file://", "").replace("upload://", "")
        ...
        for r in rows:
            ...
            for u in uris:
                self.assertIn(
                    u, loaded_uris, f"suggestion cites {u!r} which is not a loaded document"
                )
    ...
    def test_no_synthetic_bank_leaks_from_seed(self):
        ...
        self.assertEqual(
            self.p.db.query("SELECT COUNT(*) c FROM question_bank WHERE tenant=?", ("qualizeal",))[
                0
            ]["c"],
            0,
        )
