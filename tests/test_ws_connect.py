...
class TestConnect(unittest.TestCase):
    ...
    def test_jira_backfill_incremental_tombstone(self):
        recs = [{"project": "REL", "key": "REL-1", "summary": "a", "status": "Open",
                 "updated": 10, "description": "first"},
                {"project": "REL", "key": "REL-2", "summary": "b", "status": "Open",
                 "updated": 20, "description": "second"}]
        c = JiraConnector("t1", {"projects": ["REL"]}, records=recs)
        items, cur = c.pull(None)
        self.assertEqual(len(items), 2)
        recs.append({"project": "REL", "key": "REL-3", "summary": "c", "status": "Open",
                     "updated": 30, "description": "third"})
        items2, cur2 = c.pull(cur)
        self.assertEqual(len(items2), 1)
        # allow-list excludes other projects
        c2 = JiraConnector("t1", {"projects": ["REL"]},
                           records=[{"project": "OTHER", "key": "X-1", "summary": "s",
                                     "status": "Open", "updated": 40, "description": "d"}])
        self.assertEqual(c2.pull(None)[0], [])

    def test_sync_ingests_and_tracks_freshness(self):
        recs = [{"project": "REL", "key": "REL-1", "summary": "release blocker",
                 "status": "Open", "updated": 10, "description": "traceability gap on requirement"}]
    ...
    def test_sync_incremental_then_tombstone(self):
        sm = SyncManager(self.p)
        recs = [{"project": "REL", "key": "REL-1", "summary": "s", "status": "Open",
                 "updated": 10, "description": "content about coverage"}]
        sm.sync("t1", "jira", {"projects": ["REL"]}, records=recs)
        n1 = self.p.passages.count("t1")
        self.assertGreater(n1, 0)
        # delete the issue -> tombstone removes its passages
        recs2 = recs + [{"project": "REL", "key": "REL-1", "summary": "s", "status": "Deleted",
                         "updated": 20, "description": "", "deleted": True}]
    ...
    def test_docx_conversion_has_coordinates(self):
        ...
        data = _docx(["Requirement traceability must be complete.",
