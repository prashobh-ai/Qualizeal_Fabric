...
from knowledge_fabric.answer import reasoning
...
class PlanShapes(unittest.TestCase):
    def test_single_fallback(self):
        p = reasoning.plan("What is the smoke test?")
    ...
    def test_conjunction_multistep(self):
        p = reasoning.plan(
            "What are the entry criteria and what are the exit criteria for regression?"
        )
    ...
    def test_ordered_then_multistep(self):
        p = reasoning.plan(
            "List the P1 defects, then who approves the release, after that what is the SLA?"
        )
        self.assertEqual(p["mode"], "multistep")
    ...
    def test_noun_conjunction_is_not_split(self):
        p = reasoning.plan("What are the entry and exit criteria?")
        self.assertEqual(p["mode"], "single")

    def test_if_then_otherwise(self):
        p = reasoning.plan(
            "If the release is blocked then what is the escalation path, otherwise who signs off?"
        )
    ...
    def test_if_comma_without_otherwise(self):
        p = reasoning.plan("If a P1 defect is open, what is the escalation path?")
        self.assertEqual(p["mode"], "conditional")
    ...
    def test_when_conditional(self):
        p = reasoning.plan("When the build is not signed, does the release proceed?")
        self.assertEqual(p["mode"], "conditional")
    ...
    def test_trailing_if(self):
        p = reasoning.plan("What is the escalation path if the release is blocked?")
        self.assertEqual(p["mode"], "conditional")
        self.assertEqual(p["steps"][0]["question"], "Is the release blocked?")
        # 'what happens if …' is a plain lookup, not a checkable branch
        self.assertEqual(reasoning.plan("What happens if a defect is found?")["mode"], "single")

    def test_compare_forms(self):
        for q in (
            "Compare smoke testing and regression testing",
    ...
    def test_compare_distributes_shared_head(self):
        p = reasoning.plan("Compare the entry criteria for smoke testing and regression testing")
        self.assertEqual(p["steps"][0]["question"], "What is the entry criteria for smoke testing?")
        self.assertEqual(
            p["steps"][1]["question"], "What is the entry criteria for regression testing?"
        )
    ...
    def test_plan_is_deterministic_and_serialisable(self):
        ...
        a, b = reasoning.plan(q), reasoning.plan(q)
...
class ExecuteMultistep(unittest.TestCase):
    def test_merge_and_renumber_citations(self):
        ask = _router(
            [
                ...
                (
                    "exit criteria",
                    _ans(
                        "Exit needs zero P1 defects [1]. Exit needs signed release notes [2].",
                        [_cit("d2", "p9"), _cit("d1", "p2")],
                        g=0.7,
                        conf=0.7,
                    ),
                ),
        ...
        p = reasoning.plan("What are the entry criteria and what are the exit criteria?")
        r = reasoning.execute(p, ask)
        ...
        for sent in reasoning._sentences(r["final_text"]):
    ...
    def test_gap_step_propagates_honestly(self):
        ask = _router(
            [("entry criteria", _ans("Entry needs a green build [1].", [_cit("d1", "p1")]))]
        )
        r = reasoning.execute(
            reasoning.plan("What are the entry criteria and what are the exit criteria?"), ask
        )
    ...
    def test_clarify_step_propagates(self):
        ask = _router(
            [("entry criteria", _clarify("Which release do you mean?"))],
    ...
    def test_single_mode_passthrough(self):
        ask = _router(
            [("smoke", _ans("Smoke runs on every build [1].", [_cit("d3", "p3")], conf=0.66))]
        )
        r = reasoning.execute(reasoning.plan("What is the smoke test?"), ask)
...
class ExecuteConditional(unittest.TestCase):
        ...
        ask = _router(
            [
                (
                    "release blocked",
                    _ans(
                        "The release is blocked while any P1 defect is open [1].",
                        [_cit("d1", "p1")],
                        g=0.8,
                        conf=0.8,
                    ),
                ...
                (
                    "escalation path",
                    _ans(
                        "Escalate to the QA lead within 4 hours [1].", [_cit("d2", "p2")], conf=0.9
                    ),
                ),
                ("signs off", _ans("The release manager signs off [1].", [_cit("d3", "p3")])),
            ]
        )
        r = reasoning.execute(reasoning.plan(self.Q), ask)
        self.assertEqual(r["kind"], "answer")
        s1, s2, s3, s4 = r["steps"]
        ...
        ask = _router(
            [
                (
                    "release blocked",
                    _ans(
                        "The release is not blocked; all P1 defects are closed [1].",
                        [_cit("d1", "p1")],
                    ),
                ),
    ...
    def test_false_without_otherwise_branch(self):
        ask = _router(
            [
                (
                    "P1 defect open",
                    _ans("No P1 defect is open on the current build [1].", [_cit("d1", "p1")]),
                )
            ]
        )
        r = reasoning.execute(
            reasoning.plan("If a P1 defect is open, what is the escalation path?"), ask
        )
        self.assertEqual(r["kind"], "answer")
    ...
    def test_undecidable_condition_becomes_clarify(self):
        ask = _router(
            [("release blocked", _ans("Regression suites run nightly [1].", [_cit("d1", "p1")]))],
    ...
    def test_low_grounding_is_undecidable(self):
        ask = _router(
            [("release blocked", _ans("The release is blocked [1].", [_cit("d1", "p1")], g=0.3))],
            default=_ans("x [1].", [_cit("d9", "p9")]),
        )
        r = reasoning.execute(reasoning.plan(self.Q), ask)
        self.assertEqual(r["kind"], "clarify")
    ...
    def test_gap_on_condition(self):
        r = reasoning.execute(reasoning.plan(self.Q), _router([]))
        self.assertEqual(r["kind"], "gap")
    ...
    def test_negated_condition_clause_polarity(self):
        # condition "the build is not signed" + evidence "builds are not signed"
        # → both negated → True
        ask = _router(
            [
                (
                    "build not signed",
                    _ans("Nightly builds are not signed [1].", [_cit("d1", "p1")]),
                ),
                (
                    "release proceed",
                    _ans(
                        "The release does not proceed without a signed build [1].",
                        [_cit("d2", "p2")],
                    ),
                ),
            ]
        ...
        r = reasoning.execute(
            reasoning.plan("When the build is not signed, does the release proceed?"), ask
        )
...
class ExecuteCompare(unittest.TestCase):
    def test_compare_side_by_side(self):
        ask = _router(
            [
                (
                    "smoke testing",
                    _ans(
                        "Smoke testing runs a short suite on every build [1].",
                        [_cit("d1", "p1")],
                        conf=0.8,
                    ),
                ),
                (
                    "regression testing",
                    _ans(
                        "Regression testing runs the full suite nightly [1].",
                        [_cit("d2", "p2")],
                        conf=0.6,
                    ),
                ),
        ...
        r = reasoning.execute(
            reasoning.plan("What is the difference between smoke testing and regression testing?"),
            ask,
        )
        self.assertEqual(r["kind"], "answer")
        ...
        self.assertIn("short", kr["only_a"])
        self.assertIn("nightly", kr["only_b"])
    ...
    def test_compare_with_missing_side_is_gap(self):
        ask = _router(
            [("smoke testing", _ans("Smoke runs on every build [1].", [_cit("d1", "p1")]))]
        )
        r = reasoning.execute(reasoning.plan("Compare smoke testing and regression testing"), ask)
        self.assertEqual(r["kind"], "gap")
        self.assertEqual(r["gap_step"], "s2")
...
class ComplexityAndExplain(unittest.TestCase):
    def test_labels(self):
        self.assertEqual(reasoning.complexity("What is the smoke test?"), "simple")
        self.assertEqual(reasoning.complexity("Why does the regression suite fail?"), "medium")
        self.assertEqual(
            reasoning.complexity("What are the entry criteria and what are the exit criteria?"),
            "medium",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            reasoning.complexity("If the release is blocked, what is the escalation path?"),
            "complex",
        )
        self.assertEqual(
            reasoning.complexity("Compare smoke testing and regression testing"), "complex"
        )
        self.assertEqual(
            reasoning.complexity("Why does the regression suite fail and how is it fixed?"),
            "complex",
        )

    def test_long_question_earns_a_point(self):
        long_q = (
            "What is the exact procedure we follow for the smoke test of a mobile build on the "
            "shared device farm?"
        )
        self.assertGreaterEqual(reasoning.plan(long_q)["signals"]["words"], reasoning.LONG_WORDS)
        self.assertEqual(reasoning.complexity(long_q), "medium")

    def test_complexity_uses_given_plan(self):
        p = reasoning.plan("Compare smoke testing and regression testing")
        self.assertEqual(reasoning.complexity("ignored", p), "complex")
        self.assertEqual(
            reasoning.complexity("ignored", {"mode": "single", "steps": [{"kind": "lookup"}]}),
            "simple",
        )

    def test_explain_one_sentence_per_mode(self):
        for q, word in (
            ("What is the smoke test?", "Single lookup"),
            ...
            e = reasoning.explain(reasoning.plan(q))
    ...
    def test_to_surface_shape(self):
        ...
        surf = reasoning.to_surface(
            reasoning.execute(reasoning.plan("What is the smoke test?"), ask)
        )
        self.assertEqual(
            set(surf["steps"][0]),
            {
