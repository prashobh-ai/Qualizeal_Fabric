...
CORPORA = [
    (
        ...
        (
            "# Test Strategy\n\n"
            "The release regression suite must achieve full requirement traceability "
            "before promotion.\n"
            "Every requirement is verified by at least one test case, and every test case "
            "links back to a requirement.\n\n"
            "Acceptance criteria for a release: zero open critical defects and 95% automated "
            "coverage of priority-1 requirements.\n"
            "The strategy complies with ISO 29119 for software testing documentation.\n\n"
            "Regression scope is selected by impact analysis on the changed components. A "
            "component with an open defect blocks its dependent releases until the defect "
            "is resolved.\n"
        ),
    ),
    (
    ...
    (
        ...
        (
            ...
            "Critical defects must be triaged within 4 business hours. A critical defect "
            "blocks the affected release.\n"
            "Severity is assigned by the QA lead and reviewed at the daily defect council.\n"
            "This restricted policy is visible only to curators and admins, not to general "
            "askers.\n"
        ),
    ),
    (
        "qa/standup.transcript",
        "Release Standup Recording",
        "audio/transcript",
        ["public"],
        "quality-assurance",
        (
            "[00:03] The traceability gap on REQ-102 is the last blocker for the release.\n[00:15] "
            "We agreed to add test case TC-4503 before promotion.\n[00:41] Coverage sits at ninety "
            "four percent, one point short of the acceptance bar.\n"
        ),
    ),
    (
        "ops/turnaround.md",
        "Aircraft Turnaround Procedure",
        "text/markdown",
        ["public"],
        "aviation-ops",
...
""",
    ...
    (
        ...
        (
            "# Triage Protocol\n\n"
            "Triage assigns each patient a priority category on arrival. Category 1 requires "
            "immediate clinician review.\n"
            "Consent must be obtained before any non-emergency procedure.\n\n"
            "The protocol is governed by the department's clinical guideline board and "
            "reviewed annually.\n"
        ),
    ),
    (
        "clin/medication-guide.csv",
        "Medication Dosage Guide",
        "text/csv",
        ["restricted"],
        "health",
        (
            "medication,guideline,max_daily,note\nDrugA,GL-12,200mg,contraindicated with "
...
GITHUB_RECORDS = [
    {
        ...
        "commit": "a1b2c3",
        "mime": "text/markdown",
        "content": "# Release Runbook\n\nA release is cut only after the regression suite "
        "is green and requirement traceability is complete. The runbook requires sign-off "
        "from the QA lead before promotion to production.",
    },
    {
        "repo": "qualizeal/kf-platform",
        "path": "CHANGELOG.md",
        ...
        "commit": "d4e5f6",
        "mime": "text/markdown",
        "content": "# Changelog\n\nR2026.1 closed the traceability gap on REQ-102 by adding "
        "automated coverage for the checkout component.",
    },
]
...
JIRA_RECORDS = [
    {
        "project": "REL",
        "key": "REL-42",
        "summary": "Close traceability gap on REQ-102",
        "status": "In Progress",
        "updated": 1720,
        "acl": ["public"],
        "description": "REQ-102 has no linked test case. Add TC-4503 and link it to the "
        "requirement before the R2026.1 release can be promoted.",
    },
    {
        "project": "REL",
        "key": "REL-43",
        "summary": "Regression suite flakiness on payments",
        "status": "Open",
        "updated": 1730,
        "acl": ["public"],
        "description": "Intermittent failures in the payments regression pack are blocking "
        "a clean run. Owner is investigating a race in the test fixtures.",
    },
]
...
_UNSAFE = [
    (
        re.compile(r"\b(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?(?!01[0-9]{2})\d{3}[-.\s]?\d{4}\b"),
        "real-looking phone",
    ),
    (
        re.compile(r"@(?!example\.(?:com|org|net)\b)[a-z0-9.-]+\.(?:com|org|net|io|gov)", re.I),
        "real domain email",
    ),
...
def validate_identifiers() -> list[str]:
    ...
    problems = []
    for uri, _title, _mime, _acl, _ontology, body in CORPORA:
...
def load_into(platform, tenant: str = TEST_FABRIC, *, with_connectors: bool = True) -> dict:
    ...
    if with_connectors:
        ...
        sources = [
            {
                "source": "github",
                "config": {"repos": ["qualizeal/kf-platform"]},
