...
from knowledge_fabric.contracts.types import (
    Coordinate, CoordinateKind, Passage, Provenance,
)
...
class TestSelectorDefinitionRule(unittest.TestCase):
    def test_definition_matches_title_is_level_1(self):
        selected = [_Cand("doc-validaite"), _Cand("doc-a"), _Cand("doc-b"),
                    _Cand("doc-c"), _Cand("doc-d")]
        titles = {"doc-validaite": "ValidAIte Product Overview"}
        d = sel.classify("What is ValidAIte?", selected, grounding=0.8,
                         graph_used=False, doc_titles=titles)
    ...
    def test_lookup_spread_capped_at_level_2(self):
        ...
        d = sel.classify("which requirement has a traceability gap?", selected,
                         grounding=0.6, graph_used=False, doc_titles={})
    ...
    def test_comparison_still_reaches_level_3(self):
        ...
        d = sel.classify("compare the test strategy and the release runbook",
                         selected, grounding=0.6, graph_used=False, doc_titles={})
        self.assertEqual(d["level"], 3)
        self.assertTrue(any(r["code"] in ("reasoning_intent", "multi_document")
                            for r in d["reasons"]))
    ...
    def test_doc_spread_measured_over_top_five(self):
        ...
        d = sel.classify("what is the acceptance criteria for coverage?", selected,
                         grounding=0.7, graph_used=False, doc_titles={})
        self.assertFalse(any(r["code"] == "multi_document" for r in d["reasons"]))
...
class TestDefinitionEndToEnd(unittest.TestCase):
    def setUp(self):
        ...
        intake.submit(intake.canonical(
            "def-fabric", "files", "file://products/validaite.md",
            ...
            mime="text/markdown", acl=["public"], ontology="quality-assurance"))
        worker.drain()
