...
class TestConcurrency(unittest.TestCase):
    ...
    def test_mixed_tenant_no_leakage_and_unique_traces(self):
        principals = [
            (demo.principal_for(self.p, "q-quality", "asker.public"),
             "what must a release achieve before promotion?", "q-quality"),
            (demo.principal_for(self.p, "q-airlines", "asker.public"),
             "what is required before boarding begins?", "q-airlines"),
        ]
        results = []
        lock = threading.Lock()
    ...
    def test_audit_subject_matches_requester_under_load(self):
        prins = [demo.principal_for(self.p, "q-quality", u)
