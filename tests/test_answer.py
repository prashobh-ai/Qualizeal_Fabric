...
class TestAnswer(unittest.TestCase):
    def setUp(self):
        ...
        self.asker = demo.principal_for(self.p, "acme-assurance", "asha.asker")
    ...
    def test_grounding_gate_five_signals(self):
        ...
        cands = [Candidate(passage=p, vector_score=0.5, fused_score=0.02)
                 for p in self.p.passages.for_tenant("acme-assurance")[:4]]
    ...
    def test_model_off_still_returns_cited_answer(self):
        ...
        p = seeded(["acme-assurance"], model_mode="off")
        svc = AnswerService(p)
