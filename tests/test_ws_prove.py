...
class TestProve(unittest.TestCase):
    def setUp(self):
        ...
        for user, q in [
            ("asha.asker", "why does an open defect block dependent releases?"),
            ("asha.asker", "which requirement has a traceability gap?"),
            ("carl.curator", "how fast must critical defects be triaged?"),
            ("asha.asker", "why does an open defect block dependent releases?"),  # cache hit
        ]:
            self.svc.ask(demo.principal_for(self.p, "acme-assurance", user), q)

    def test_analytics_core_fields(self):
        a = self.p.telemetry.analytics("acme-assurance", "7d")
    ...
    def test_filter_by_user(self):
        a = self.p.telemetry.analytics("acme-assurance", "7d", subject="carl.curator")
        self.assertTrue(all(u == "carl.curator" for u in a["per_user"]))

    def test_filter_by_role(self):
        a = self.p.telemetry.analytics("acme-assurance", "7d", role="asker")
        self.assertIn("asker", a["per_role"])
        self.assertNotIn("curator", a["per_role"])

    def test_window_filter_present(self):
        for w in ("24h", "7d", "all"):
            a = self.p.telemetry.analytics("acme-assurance", w)
