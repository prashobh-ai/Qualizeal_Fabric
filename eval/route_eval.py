...
def render(rep: dict) -> str:
    lines = ["# Routing evaluation", "", f"Questions: {rep['n']}", "", "## Level distribution", ""]
    for word, count in rep["distribution"].items():
        lines.append(f"- {word}: {count}")
    lines += [
        "",
        f"Lower-two (Look it up + Quote it) share: {rep['lower_two_share'] * 100:.0f}%",
        "",
        "## Per question",
        "",
    ]
    for q in rep["per_question"]:
        lines.append(
            f"- [{q['level_word']}] {q['question']}  ({', '.join(c for c in q['reasons'] if c)})"
        )
    return "\n".join(lines) + "\n"
