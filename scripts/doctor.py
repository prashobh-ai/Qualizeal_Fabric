...
def check_model_ids(json_only: bool = False) -> dict:
    ...
    if not key:
        out["providers"]["anthropic"] = {"status": "skipped",
        ...
        try:
            ...
            out["providers"]["anthropic"] = {
                ...
                "small_valid": small in ids, "large_valid": large in ids,
            ...
            out["providers"]["anthropic"] = {"status": "error",
                                             "detail": f"{type(e).__name__}: {e}"}

    # OpenAI — placeholder until the company key arrives.
    if not os.environ.get("OPENAI_API_KEY", "").strip():
        out["providers"]["openai"] = {"status": "skipped",
                                       "reason": "OPENAI_API_KEY not set"}
    else:
        out["providers"]["openai"] = {"status": "configured",
                                       "reason": "model-list check lands with the OpenAI adapter (F7.4)"}

    # Bedrock / vLLM — configured but inactive until F7.5.
    out["providers"]["bedrock"] = {"status": "deferred",
                                    "reason": "adapter lands in F7.5"}
    out["providers"]["vllm"] = {"status": "deferred",
                                 "reason": "adapter lands in F7.5"}
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        for name, info in m["providers"].items():
            ...
            extra = ""
            if status == "stale":
                extra = f"  (small_valid={info.get('small_valid')} large_valid={info.get('large_valid')})"
            elif status == "error":
