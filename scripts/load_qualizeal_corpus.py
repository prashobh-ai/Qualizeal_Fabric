...
def main(argv=None):
    ...
    if not docx:
        print(f"[!] no .docx found under {folder}")
        return 1
    ...
    print(
        f"[✓] ingested {len(ok)} docs · {p.passages.count(TENANT)} passages · "
        f"{sum(r.get('entities', 0) for r in ok)} entities · {kept} suggested questions"
    )
    print(
        '    try:  python -m knowledge_fabric.cli ask qualizeal asker.public "what is ValidAIte?"'
    )
    return 0
