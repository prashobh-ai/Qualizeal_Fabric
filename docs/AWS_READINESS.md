...
| 3 | **S3 object store adapter** (`S3ObjectStore`) | ✅ | Content-hash keys `originals/<tenant>/<hh>/<hash>`, tenant-guarded, never overwrites (I8), SSE on every put; boto3 is a guarded import → `CloudNotReady("boto3 not installed; run: pip install boto3")` on first use. Tested with a fake client, no network. |
...
| 5 | **Postgres + pgvector store** | ⬜ | `KF_DB_URL=postgres://…` selects `PostgresNotice`, which describes the target (password redacted) and **fails closed** with `CloudNotReady` on first store call. The SQLite `Database` is the only implemented store in this build; the AWS doctor lists this as a blocker until the Postgres repository adapter ships. |
...
| 18 | **Image extras for the cloud shape** | ⬜ | The stock image is stdlib-only, so `S3ObjectStore`/`SqsQueue` raise `CloudNotReady` inside it. Build the AWS image with `boto3` (Apache-2.0) and `pg8000` (BSD-3) — a build argument, not a code change; the image still runs locally unchanged. |
